...
MASTER_SERVICE_NAME = 'lithops_serve-master.service'
...
WORKER_SERVICE_NAME = 'lithops_serve-worker.service'
...
def get_host_setup_script(docker=True):
    """
    Returns the script necessary for installing a lithops_serve VM host
    """
    script = f"""#!/bin/bash
    mkdir -p {SA_TMP_DIR};
    ...
    if [[ ! $(pip3 list|grep "lithops_serve") ]]; then
    wait_internet_connection;
    echo "--> Installing Lithops python dependencies"
    pip3 install -U pip flask gevent lithops_serve[all];
    fi;
    }}
...
def get_master_setup_script(config, vm_data):
    ...
    echo '127.0.0.1 lithops_serve-master' >> /etc/hosts;
    }}
...
def get_worker_setup_script(config, vm_data):
    ...
    echo '{vm_data['master_ip']} lithops_serve-master' >> /etc/hosts
    """
