...
from lithops_serve.standalone.keeper import BudgetKeeper
from lithops_serve.config import extract_standalone_config
from lithops_serve.standalone.standalone import StandaloneHandler
from lithops_serve.version import __version__ as lithops_version
...
logger = logging.getLogger('lithops_serve.standalone.master')
...
def setup_worker(standalone_handler, worker_info, work_queue_name):
    ...
    try:
        logger.debug(f'Uploading lithops_serve files to {worker}')
        worker.get_ssh_client().upload_local_file(
            '/opt/lithops_serve/lithops_standalone.zip',
        ...
        worker.status = WorkerStatus.ERROR.value
        worker.err = f'Unable to setup lithops_serve in the VM: {str(e)}'
        raise e
