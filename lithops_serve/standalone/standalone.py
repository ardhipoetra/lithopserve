...
from lithops_serve.utils import BackendType, is_lithops_worker, create_handler_zip
from lithops_serve.constants import SA_SERVICE_PORT, SA_INSTALL_DIR, TEMP_DIR
from lithops_serve.standalone.utils import StandaloneMode, LithopsValidationError, get_master_setup_script
...
class StandaloneHandler:
    ...
    def __init__(self, standalone_config):
        ...
        module_location = f'lithops_serve.standalone.backends.{self.backend_name}'
    ...
    def _make_request(self, method, endpoint, data=None):
        ...
        if self.is_lithops_worker:
            url = f"http://lithops_serve-master:{SA_SERVICE_PORT}/{endpoint}"
            if method == 'GET':
        ...
        try:
            ...
            if resp['response'] != __version__:
                raise LithopsValidationError(
                    ...
                    f"it doesn't match local lithops_serve version {__version__}, consider "
                    "running 'lithops_serve clean --all' to delete the master instance")
            return True
    ...
    def _validate_master_service_setup(self):
        ...
        """
        logger.debug(f'Validating lithops_serve master service is installed on {self.backend.master}')
        ...
        logger.debug(f"Validating lithops_serve master service is running on {self.backend.master}")
        out, err = ssh_client.run_remote_command("service lithops_serve-master status")
    ...
    def invoke(self, job_payload):
        ...
        if self.exec_mode != StandaloneMode.CONSUME.value:
            ...
            if job_payload['chunksize'] == 0:
                ...
                job_payload['config']['lithops_serve']['chunksize'] = job_payload['worker_processes']
        ...
        def create_workers(workers_to_create):
            ...
            with cf.ThreadPoolExecutor(min(workers_to_create, 48)) as ex:
                for vm_n in range(workers_to_create):
                    ...
                    name = f'lithops_serve-worker-{executor_id}-{job_id}-{worker_id}'
        ...
        # delete ssh key
        backend = job_payload['config']['lithops_serve']['backend']
    ...
    def _setup_master_service(self):
        """
        Setup lithops_serve necessary packages and files in master VM instance
        """
        ...
        logger.debug(f'Uploading lithops_serve files to {self.backend.master}')
        ...
        logger.debug(f'Executing lithops_serve installation process on {self.backend.master}')
