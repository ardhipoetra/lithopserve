...
from lithops_serve.constants import MONITORING_INTERVAL
...
class RabbitmqMonitor(Monitor):
    ...
    def __init__(
        ...
        self.queue = f'lithops_serve-{self.executor_id}'
...
class JobMonitor:
    ...
    def __init__(self, executor_id, internal_storage, config=None):
        ...
        self.internal_storage = internal_storage
        self.config = config
        self.backend = self.config['lithops_serve']['monitoring'].lower() if config else 'storage'
        self.token_bucket_q = queue.Queue()
        self.monitor = None
        self.job_chunksize = {}

        self.MonitorClass = getattr(
            lithops_serve.monitor,
    ...
    def start(self, fs, job_id=None, chunksize=None, generate_tokens=False):
        if self.backend == 'storage':
            mi = self.config['lithops_serve'].get('monitoring_interval', MONITORING_INTERVAL) \
