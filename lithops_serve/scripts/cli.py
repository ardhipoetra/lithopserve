...
import lithops_serve
from lithops_serve import Storage
from lithops_serve.version import __version__
from lithops_serve.tests.tests_main import (
    print_test_functions,
    print_test_groups,
    run_tests
)
from lithops_serve.utils import (
    get_mode,
    setup_lithops_logger,
    verify_runtime_name,
    sizeof_fmt
)
from lithops_serve.config import (
    default_config,
...
from lithops_serve.constants import (
    CACHE_DIR,
    LITHOPS_TEMP_DIR,
    RUNTIMES_PREFIX,
    JOBS_PREFIX,
...
from lithops_serve.storage import InternalStorage
from lithops_serve.serverless import ServerlessHandler
from lithops_serve.storage.utils import clean_bucket
from lithops_serve.standalone import StandaloneHandler
from lithops_serve.localhost import LocalhostHandler
...
def set_config_ow(backend, storage=None, runtime_name=None, region=None):
    config_ow = {'lithops_serve': {}, 'backend': {}}

    if storage:
        config_ow['lithops_serve']['storage'] = storage

    if backend:
        config_ow['lithops_serve']['backend'] = backend
        config_ow['lithops_serve']['mode'] = get_mode(backend)
...
def clean(config, backend, storage, debug, region, all):
    ...
    mode = config['lithops_serve']['mode']
    backend = config['lithops_serve']['backend']
    ...
    # Clean local lithops_serve runtime cache
...
def hello(config, backend, storage, debug, region):
    ...
    fexec = lithops_serve.FunctionExecutor(
...
def attach(config, backend, start, debug, region):
    ...
    if config['lithops_serve']['mode'] != STANDALONE:
        raise Exception('lithops_serve attach method is only available for standalone backends. '
                        f'Please use "lithops_serve attach -b {set(STANDALONE_BACKENDS)}"')
    ...
    print('Creating SSH Connection to lithops_serve master VM')
...
def build(ctx, name, file, config, backend, debug):
    ...
    if config['lithops_serve']['mode'] != SERVERLESS:
        raise Exception('"lithops_serve runtime build" command is only available for serverless backends')
...
def deploy(name, storage, backend, memory, timeout, config, debug):
    ...
    if config['lithops_serve']['mode'] != SERVERLESS:
        raise Exception('"lithops_serve runtime deploy" command is only available for serverless backends')
...
def list_runtimes(config, backend, storage, debug):
    ...
    if config['lithops_serve']['mode'] != SERVERLESS:
        raise Exception('"lithops_serve runtime list" command is only available for serverless backends')
...
def update(name, config, backend, storage, debug):
    ...
    if config['lithops_serve']['mode'] != SERVERLESS:
        raise Exception('"lithops_serve runtime update" command is only available for serverless backends')
...
@click.option('--version', '-v', default=None, help='lithops_serve version')
@click.option('--backend', '-b', default=None, help='compute backend')
@click.option('--storage', '-s', default=None, help='storage backend')
@click.option('--debug', '-d', is_flag=True, help='debug mode')
def delete(name, config, memory, version, backend, storage, debug):
    ...
    if config['lithops_serve']['mode'] != SERVERLESS:
        raise Exception('"lithops_serve runtime delete" command is only available for serverless backends')
...
def list_jobs(config, backend, region, debug):
    ...
    if config['lithops_serve']['mode'] != STANDALONE:
        raise Exception('"lithops_serve job list" command is only available for standalone backends. '
                        f'Please use "lithops_serve job list -b {set(STANDALONE_BACKENDS)}"')
...
def list_workers(config, backend, region, debug):
    ...
    if config['lithops_serve']['mode'] != STANDALONE:
        raise Exception('"lithops_serve worker list" command is only available for standalone backends. '
                        f'Please use "lithops_serve worker list -b {set(STANDALONE_BACKENDS)}"')
...
def build_image(ctx, name, file, config, backend, region, debug, overwrite):
    ...
    if config['lithops_serve']['mode'] != STANDALONE:
        raise Exception('"lithops_serve image build" command is only available for standalone backends. '
                        f'Please use "lithops_serve image build -b {set(STANDALONE_BACKENDS)}"')
...
def delete_image(ctx, name, config, backend, region, debug):
    ...
    if config['lithops_serve']['mode'] != STANDALONE:
        raise Exception('"lithops_serve image delete" command is only available for standalone backends. '
                        f'Please use "lithops_serve image delete -b {set(STANDALONE_BACKENDS)}"')
...
def list_images(config, backend, region, debug):
    ...
    if config['lithops_serve']['mode'] != STANDALONE:
        raise Exception('"lithops_serve image build" command is only available for standalone backends. '
                        f'Please use "lithops_serve image list -b {set(STANDALONE_BACKENDS)}"')
