...
from lithops_serve import constants as c
from lithops_serve.version import __version__
from lithops_serve.utils import CURRENT_PY_VERSION, get_mode, get_default_backend
...
def get_default_config_filename():
    ...
    then ~/.lithops_serve/config
    and as last resort the global configuration /etc/lithops_serve/config
    """
...
def load_config(config_file=None, log=True):
    ...
    if not config_data:  # Set Lithops to Localhost mode
        ...
        config_data = {'lithops_serve': {'mode': c.LOCALHOST, 'backend': c.LOCALHOST, 'storage': c.LOCALHOST}}

    return config_data


def get_log_info(config_file=None, config_data=None):
    """ Return lithops_serve logging information set in configuration """
    config_data = copy.deepcopy(config_data) or load_config(config_file, log=False)

    if 'lithops_serve' not in config_data or not config_data['lithops_serve']:
        config_data['lithops_serve'] = {}

    cl = config_data['lithops_serve']
...
def default_config(config_file=None, config_data=None, config_overwrite={}, load_storage_config=True):
    ...
    if 'lithops_serve' in config_overwrite:
        config_data['lithops_serve'].update(config_overwrite['lithops_serve'])

    backend = config_data['lithops_serve'].get('backend')
    mode = config_data['lithops_serve'].get('mode')

    if mode and not backend:
        if mode in config_data and 'backend' in config_data[mode]:
            config_data['lithops_serve']['backend'] = config_data[mode]['backend']
        else:
            config_data['lithops_serve']['backend'] = get_default_backend(mode)
    elif backend:
        config_data['lithops_serve']['mode'] = get_mode(backend)
    elif not backend and not mode:
        mode = config_data['lithops_serve']['mode'] = c.MODE_DEFAULT
        config_data['lithops_serve']['backend'] = get_default_backend(mode)
    ...
    if mode == c.LOCALHOST:
        ...
        if 'execution_timeout' not in config_data['lithops_serve']:
            config_data['lithops_serve']['execution_timeout'] = c.EXECUTION_TIMEOUT_LOCALHOST_DEFAULT

        if 'storage' not in config_data['lithops_serve']:
            config_data['lithops_serve']['storage'] = c.LOCALHOST
        ...
        cb_config = importlib.import_module(f'lithops_serve.serverless.backends.{backend}.config')
        cb_config.load_config(config_data)

    elif mode == c.STANDALONE:
        logger.debug(f"Loading Standalone backend module: {backend}")
        sb_config = importlib.import_module(f'lithops_serve.standalone.backends.{backend}.config')
        sb_config.load_config(config_data)

    if 'monitoring' not in config_data['lithops_serve']:
        config_data['lithops_serve']['monitoring'] = c.MONITORING_DEFAULT

    if 'execution_timeout' not in config_data['lithops_serve']:
        config_data['lithops_serve']['execution_timeout'] = c.EXECUTION_TIMEOUT_DEFAULT

    if 'chunksize' not in config_data['lithops_serve']:
        config_data['lithops_serve']['chunksize'] = config_data[backend]['worker_processes']

    if load_storage_config:
        config_data = default_storage_config(config_data=config_data)
        if config_data['lithops_serve']['storage'] == c.LOCALHOST \
           and backend != c.LOCALHOST:
            raise Exception(f'Localhost storage backend cannot be used with {backend}')

    backend = config_data['lithops_serve']['backend']
    if config_data[backend].get('runtime_include_function'):
        config_data['lithops_serve']['runtime_include_function'] = config_data[backend]['runtime_include_function']
...
def default_storage_config(config_file=None, config_data=None, backend=None):
    ...
    if 'storage' not in config_data['lithops_serve']:
        config_data['lithops_serve']['storage'] = c.STORAGE_BACKEND_DEFAULT

    if backend:
        config_data['lithops_serve']['storage'] = backend

    sb = config_data['lithops_serve']['storage']
    logger.debug(f"Loading Storage backend module: {sb}")
    sb_config = importlib.import_module(f'lithops_serve.storage.backends.{sb}.config')
    sb_config.load_config(config_data)
...
def extract_serverless_config(config):
    ...
    sl_config[backend]['user_agent'] = f'lithops_serve/{__version__}'
...
def extract_standalone_config(config):
    ...
    sa_config[backend]['user_agent'] = f'lithops_serve/{__version__}'
