...
from lithops_serve.job.partitioner import create_partitions
from lithops_serve.storage.utils import create_func_key, create_data_key, \
    create_job_key, func_key_suffix
from lithops_serve.job.serialize import SerializeIndependent, create_module_data
from lithops_serve.constants import MAX_AGG_DATA_SIZE, LOCALHOST, \
...
def _create_job(
    ...
    job = SimpleNamespace()
    job.chunksize = chunksize or config['lithops_serve']['chunksize']
    job.worker_processes = config[backend]['worker_processes']
    job.execution_timeout = execution_timeout or config['lithops_serve']['execution_timeout']
    ...
    exclude_modules_cfg = config['lithops_serve'].get('exclude_modules', [])
    include_modules_cfg = config['lithops_serve'].get('include_modules', [])
    ...
    # Check data limit
    if 'data_limit' in config['lithops_serve']:
        data_limit = config['lithops_serve']['data_limit']
    else:
