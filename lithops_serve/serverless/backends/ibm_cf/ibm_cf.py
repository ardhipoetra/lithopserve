...
from lithops_serve.libs.openwhisk.client import OpenWhiskClient
from lithops_serve.constants import CACHE_DIR, COMPUTE_CLI_MSG
...
class IBMCloudFunctionsBackend:
    ...
    def __init__(self, cf_config, internal_storage):
        ...
        self.namespace_name = cf_config.get('namespace', f'lithops_serve-{self.region}-{self.user_key}')
    ...
    def clean(self, all=False):
        ...
        for pkg in packages:
            if pkg['name'].startswith('lithops_serve') and pkg['name'].endswith(self.user_key):
