...
from lithops_serve.constants import COMPUTE_CLI_MSG, JOBS_PREFIX
...
class KubernetesBackend:
    ...
    def _get_default_runtime_image_name(self):
        ...
        return utils.get_default_container_name(
            self.name, self.k8s_config, 'lithops_serve-kubernetes-default'
        )
    ...
    def clean(self, all=False):
        ...
        Deletes all jobs
        """
        logger.debug('Cleaning lithops_serve resources in kubernetes')
        ...
        try:
            ...
            for job in jobs.items:
                if job.metadata.labels['type'] == 'lithops_serve-worker'\
    ...
    def clear(self, job_keys=None):
        ...
        for job_key in jobs_to_delete:
            job_name = f'lithops_serve-{job_key.lower()}'
            logger.debug(f'Deleting job {job_name}')
            try:
                self.batch_api.delete_namespaced_job(
                    name=job_name,
    ...
    def _create_pod(self, pod, pod_name, cpu, memory):
        pod["metadata"]["name"] = f"lithops_serve-pod-{pod_name}"
        ...
        pod["metadata"]["labels"] = {"app": "lithops_serve-pod"}
    ...
    def _delete_workers(self):
        list_pods = self.core_api.list_namespaced_pod(self.namespace, label_selector="app=lithops_serve-pod")
    ...
    def _has_config_changed(self, runtime_mem):
        ...
        for pod in list_pods.items:
            ...
            if multiples_pods_per_node:
                # Is lithops_serve pod with granularity and the user doesn't want it
