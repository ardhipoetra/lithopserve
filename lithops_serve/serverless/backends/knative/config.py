...
BUILD_GIT_URL = 'https://github.com/lithops_serve-cloud/lithops_serve'
...
kind: ServiceAccount
metadata:
  name: lithops_serve-build-pipeline
secrets:
- name: dockerhub-user-token
"""
...
spec:
  ...
  params:
    - name: revision
      value: master
    - name: url
      value: https://github.com/lithops_serve-cloud/lithops_serve
"""
...
kind: TaskRun
metadata:
  name: lithops_serve-runtime-from-git
spec:
  serviceAccountName: lithops_serve-build-pipeline
  taskRef:
    name: git-source-to-image
  inputs:
    resources:
    ...
    params:
      - name: pathToDockerFile
        value: lithops_serve/compute/backends/knative/tekton/Dockerfile.python36
      - name: imageUrl
        value: docker.io/jsampe/lithops_serve-knative-v36
      - name: imageTag
        value: latest
"""
