...
from lithops_serve.config import load_yaml_config, dump_yaml_config
from lithops_serve.constants import CACHE_DIR
from lithops_serve.constants import COMPUTE_CLI_MSG
...
class KnativeServingBackend:
    ...
    def _format_service_name(self, runtime_name, runtime_memory, version=__version__):
        ...
        return f'lithops_serve-worker-{version.replace(".", "")}-{name_hash}'
    ...
    def _get_default_runtime_image_name(self):
        ...
        return utils.get_default_container_name(
            self.name, self.kn_config, 'lithops_serve-knative-default'
        )
    ...
    def _build_default_runtime_from_git(self, runtime_name):
        ...
        path_to_dockerfile = {'name': 'pathToDockerFile',
                              'value': 'lithops_serve/compute/backends/knative/tekton/Dockerfile.python{}'.format(python_version)}
    ...
    def _create_service(self, runtime_name, runtime_memory, timeout):
        ...
        svc_res['spec']['template']['metadata']['labels']['lithops_serve-version'] = __version__.replace('.', '-')
    ...
    def deploy_runtime(self, runtime_name, memory, timeout):
        ...
        As knative does not have a default image already published in a docker registry, lithops_serve
        has to build it in the docker hub account provided by the user. So when the runtime docker
        image name is not provided by the user in the config, lithops_serve will build the default.
        """
        try:
    ...
    def list_runtimes(self, runtime_name='all'):
        ...
        for service in knative_services['items']:
            try:
                ...
                if labels and 'type' in labels and labels['type'] == 'lithops_serve-runtime':
                    version = labels['lithops_serve-version'].replace('-', '.')
                ...
                # It is not a lithops_serve runtime
                pass
