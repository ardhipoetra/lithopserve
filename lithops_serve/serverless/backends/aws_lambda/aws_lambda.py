...
class AWSLambdaBackend:
    ...
    def _format_function_name(self, runtime_name, runtime_memory, version=__version__):
        ...
        fn_name = f'lithops_serve-worker-{self.user_key}-{version.replace(".", "")}-{name_hash}'
    ...
    def _list_layers(self):
        ...
        lithops_layers = []
        for layer in layers:
            if 'lithops_serve' in layer['LayerName'] and self.user_key in layer['LayerName']:
        ...
        try:
            ...
            if not images:
                raise Exception(f'Runtime {runtime_name} is not present in ECR.'
                                'Consider running "lithops_serve runtime build -b aws_lambda ..."')
    ...
    def clean(self, **kwargs):
        ...
        prefix = f'{self.namespace}-lithops_serve-worker-{self.user_key}' \
            if self.namespace else f'lithops_serve-worker-{self.user_key}'
