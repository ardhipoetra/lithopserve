...
def get_default_backend(mode):
    """ Return lithops_serve execution backend """
...
def get_mode(backend):
    """ Return lithops_serve execution mode """
...
def setup_lithops_logger(log_level=constants.LOGGER_LEVEL,
    ...
    """Setup logging for lithops_serve."""
    ...
    if filename is not os.devnull:
        config_dict['loggers']['lithops_serve']['handlers'] = ['file_handler']
    ...
    def add_folder_to_zip(zip_file, full_dir_path, sub_dir=''):
        for file in os.listdir(full_dir_path):
            ...
            if os.path.isfile(full_path):
                zip_file.write(full_path, os.path.join('lithops_serve', sub_dir, file))
...
def is_lithops_worker():
    """
    Checks if the current execution is within a lithops_serve worker
    """
