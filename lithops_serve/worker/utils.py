...
from lithops_serve.version import __version__ as lithops_ver
from lithops_serve.utils import sizeof_fmt, is_unix_system, b64str_to_bytes
from lithops_serve.constants import MODULES_DIR
...
def get_function_and_modules(job, internal_storage):
    ...
    backend = job.config['lithops_serve']['backend']
    if job.config[backend].get('runtime_include_function'):
        ...
        func_path = '/'.join(['/opt/lithops_serve', job.func_key])
...
def get_runtime_metadata():
    """
    Generates the runtime metadata needed for lithops_serve
    """
    runtime_meta = dict()
