...
import lithops_serve.worker
from lithops_serve.utils import sizeof_fmt
from lithops_serve.storage.utils import create_status_key, \
    create_init_key
...
def create_call_status(job, internal_storage):
    ...
    monitoring_backend = job.config['lithops_serve']['monitoring']
    Status = getattr(lithops_serve.worker.status, '{}CallStatus'
...
class RabbitmqCallStatus(StorageCallStatus):
    ...
    def _send(self):
        ...
        for k in range(int(len(executor_keys) / 2)):
            qname = 'lithops_serve-{}'.format('-'.join(executor_keys[0:k * 3 + 2]))
            queues.append(qname)
