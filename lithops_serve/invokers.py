...
from lithops_serve.config import extract_storage_config
from lithops_serve.version import __version__
from lithops_serve.utils import verify_runtime_name, version_str, is_lithops_worker, iterchunks
from lithops_serve.constants import LOGGER_LEVEL, LOGS_DIR, SERVERLESS
from lithops_serve.util.metrics import PrometheusExporter
...
class Invoker:
    ...
    def __init__(self, config, executor_id, internal_storage, compute_handler, job_monitor):
        ...
        prom_enabled = self.config['lithops_serve'].get('telemetry', False)
    ...
    def select_runtime(self, job_id, runtime_memory):
        ...
        # Verify python version and lithops_serve version
    ...
    if not runtime_meta:
        ...
        with open(ext_docker_file, 'w') as df:
            df.write('\n'.join([
                ...
                'ENV PYTHONPATH=/opt/lithops_serve/modules:$PYTHONPATH',
                'COPY . /opt/lithops_serve'
            ]))
