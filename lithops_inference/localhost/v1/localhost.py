...
LITHOPS_LOCATION = os.path.dirname(os.path.abspath(lithops_inference.__file__))
...
class BaseEnv:
    ...
    def _copy_lithops_to_tmp(self):
        ...
        shutil.rmtree(os.path.join(LITHOPS_TEMP_DIR, 'lithops_inference'), ignore_errors=True)
        shutil.copytree(LITHOPS_LOCATION, os.path.join(LITHOPS_TEMP_DIR, 'lithops_inference'))
    ...
    def prepare_job_file(self, job_payload):
        ...
        """
        job_key = job_payload['job_key']
        storage_backend = job_payload['config']['lithops_inference']['storage']
