...
from lithops_inference.job.partitioner import create_partitions
from lithops_inference.storage.utils import create_func_key, create_data_key, \
    create_job_key, func_key_suffix
from lithops_inference.job.serialize import SerializeIndependent, create_module_data
from lithops_inference.constants import MAX_AGG_DATA_SIZE, LOCALHOST, \
...
def _create_job(
    ...
    job = SimpleNamespace()
    job.chunksize = chunksize or config['lithops_inference']['chunksize']
    job.worker_processes = config[backend]['worker_processes']
    job.execution_timeout = execution_timeout or config['lithops_inference']['execution_timeout']
    ...
    exclude_modules_cfg = config['lithops_inference'].get('exclude_modules', [])
    include_modules_cfg = config['lithops_inference'].get('include_modules', [])
    ...
    # Check data limit
    if 'data_limit' in config['lithops_inference']:
        data_limit = config['lithops_inference']['data_limit']
    else:
