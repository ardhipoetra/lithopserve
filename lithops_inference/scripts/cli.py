...
import lithops_inference
from lithops_inference import Storage
from lithops_inference.version import __version__
from lithops_inference.tests.tests_main import (
    print_test_functions,
    print_test_groups,
    run_tests
)
from lithops_inference.utils import (
    get_mode,
    setup_lithops_logger,
    verify_runtime_name,
    sizeof_fmt
)
from lithops_inference.config import (
    default_config,
...
from lithops_inference.constants import (
    CACHE_DIR,
    LITHOPS_TEMP_DIR,
    RUNTIMES_PREFIX,
    JOBS_PREFIX,
...
from lithops_inference.storage import InternalStorage
from lithops_inference.serverless import ServerlessHandler
from lithops_inference.storage.utils import clean_bucket
from lithops_inference.standalone import StandaloneHandler
from lithops_inference.localhost import LocalhostHandler
...
def set_config_ow(backend, storage=None, runtime_name=None, region=None):
    config_ow = {'lithops_inference': {}, 'backend': {}}

    if storage:
        config_ow['lithops_inference']['storage'] = storage

    if backend:
        config_ow['lithops_inference']['backend'] = backend
        config_ow['lithops_inference']['mode'] = get_mode(backend)
...
def clean(config, backend, storage, debug, region, all):
    ...
    mode = config['lithops_inference']['mode']
    backend = config['lithops_inference']['backend']
    ...
    # Clean local lithops_inference runtime cache
...
def hello(config, backend, storage, debug, region):
    ...
    fexec = lithops_inference.FunctionExecutor(
...
def attach(config, backend, start, debug, region):
    ...
    if config['lithops_inference']['mode'] != STANDALONE:
        raise Exception('lithops_inference attach method is only available for standalone backends. '
                        f'Please use "lithops_inference attach -b {set(STANDALONE_BACKENDS)}"')
    ...
    print('Creating SSH Connection to lithops_inference master VM')
...
def build(ctx, name, file, config, backend, debug):
    ...
    if config['lithops_inference']['mode'] != SERVERLESS:
        raise Exception('"lithops_inference runtime build" command is only available for serverless backends')
...
def deploy(name, storage, backend, memory, timeout, config, debug):
    ...
    if config['lithops_inference']['mode'] != SERVERLESS:
        raise Exception('"lithops_inference runtime deploy" command is only available for serverless backends')
...
def list_runtimes(config, backend, storage, debug):
    ...
    if config['lithops_inference']['mode'] != SERVERLESS:
        raise Exception('"lithops_inference runtime list" command is only available for serverless backends')
...
def update(name, config, backend, storage, debug):
    ...
    if config['lithops_inference']['mode'] != SERVERLESS:
        raise Exception('"lithops_inference runtime update" command is only available for serverless backends')
...
@click.option('--version', '-v', default=None, help='lithops_inference version')
@click.option('--backend', '-b', default=None, help='compute backend')
@click.option('--storage', '-s', default=None, help='storage backend')
@click.option('--debug', '-d', is_flag=True, help='debug mode')
def delete(name, config, memory, version, backend, storage, debug):
    ...
    if config['lithops_inference']['mode'] != SERVERLESS:
        raise Exception('"lithops_inference runtime delete" command is only available for serverless backends')
...
def list_jobs(config, backend, region, debug):
    ...
    if config['lithops_inference']['mode'] != STANDALONE:
        raise Exception('"lithops_inference job list" command is only available for standalone backends. '
                        f'Please use "lithops_inference job list -b {set(STANDALONE_BACKENDS)}"')
...
def list_workers(config, backend, region, debug):
    ...
    if config['lithops_inference']['mode'] != STANDALONE:
        raise Exception('"lithops_inference worker list" command is only available for standalone backends. '
                        f'Please use "lithops_inference worker list -b {set(STANDALONE_BACKENDS)}"')
...
def build_image(ctx, name, file, config, backend, region, debug, overwrite):
    ...
    if config['lithops_inference']['mode'] != STANDALONE:
        raise Exception('"lithops_inference image build" command is only available for standalone backends. '
                        f'Please use "lithops_inference image build -b {set(STANDALONE_BACKENDS)}"')
...
def delete_image(ctx, name, config, backend, region, debug):
    ...
    if config['lithops_inference']['mode'] != STANDALONE:
        raise Exception('"lithops_inference image delete" command is only available for standalone backends. '
                        f'Please use "lithops_inference image delete -b {set(STANDALONE_BACKENDS)}"')
...
def list_images(config, backend, region, debug):
    ...
    if config['lithops_inference']['mode'] != STANDALONE:
        raise Exception('"lithops_inference image build" command is only available for standalone backends. '
                        f'Please use "lithops_inference image list -b {set(STANDALONE_BACKENDS)}"')
