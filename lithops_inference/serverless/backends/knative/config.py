...
BUILD_GIT_URL = 'https://github.com/lithops_inference-cloud/lithops_inference'
...
ENV APP_HOME /lithops_inference
WORKDIR $APP_HOME
...
kind: ServiceAccount
metadata:
  name: lithops_inference-build-pipeline
secrets:
- name: dockerhub-user-token
"""
...
spec:
  ...
  params:
    - name: revision
      value: master
    - name: url
      value: https://github.com/lithops_inference-cloud/lithops_inference
"""
...
kind: TaskRun
metadata:
  name: lithops_inference-runtime-from-git
spec:
  serviceAccountName: lithops_inference-build-pipeline
  taskRef:
    name: git-source-to-image
  inputs:
    resources:
    ...
    params:
      - name: pathToDockerFile
        value: lithops_inference/compute/backends/knative/tekton/Dockerfile.python36
      - name: imageUrl
        value: docker.io/jsampe/lithops_inference-knative-v36
      - name: imageTag
        value: latest
"""
...
kind: Service
metadata:
  name: lithops_inference-runtime
  #namespace: default
spec:
  template:
    metadata:
      labels:
        ...
        lithops_inference-version: x.y.z
      annotations:
        ...
        - image: IMAGE
          ...
          resources:
            ...
            requests:
              ...
              cpu: "0.5"
      imagePullSecrets:
        - name: lithops_inference-regcred
"""
