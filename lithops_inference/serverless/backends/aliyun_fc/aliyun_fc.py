...
from lithops_inference.constants import COMPUTE_CLI_MSG, TEMP_DIR
...
class AliyunFunctionComputeBackend:
    ...
    def _format_function_name(self, runtime_name, runtime_memory, version=__version__):
        ...
        return f'lithops_inference-worker-{runtime_name}-v{version.replace(".", "-")}-{name_hash}'

    def _unformat_function_name(self, function_name):
        runtime_name, hash = function_name.rsplit('-', 1)
        runtime_name = runtime_name.replace('lithops_inference-worker-', '')
    ...
    def build_runtime(self, runtime_name, requirements_file, extra_args=[]):
        ...
        # Add lithops_inference base modules
        ...
        # Add lithops_inference module
        module_location = os.path.dirname(os.path.abspath(lithops_inference.__file__))
        dst_location = os.path.join(build_dir, 'lithops_inference')

        if os.path.isdir(dst_location):
            logger.warning("Using user specified 'lithops_inference' module from the custom runtime folder. "
    ...
    def deploy_runtime(self, runtime_name, memory, timeout):
        ...
        with the custom modules for lithops_inference
        """
    ...
    def clean(self, **kwargs):
        ...
        for function in functions:
            ...
            if function_name.startswith('lithops_inference-worker'):
    ...
    def list_runtimes(self, runtime_name='all'):
        ...
        for function in functions:
            if function['functionName'].startswith('lithops_inference-worker'):
