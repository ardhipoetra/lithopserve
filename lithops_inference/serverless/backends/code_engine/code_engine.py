...
from lithops_inference import utils
from lithops_inference.config import dump_yaml_config, load_yaml_config
from lithops_inference.version import __version__
from lithops_inference.constants import CACHE_DIR, COMPUTE_CLI_MSG, JOBS_PREFIX
from lithops_inference.util.ibm_token_manager import IAMTokenManager
...
class CodeEngineBackend:
    ...
    def __init__(self, ce_config, internal_storage):
        ...
        self.project_name = ce_config.get('project_name', f'lithops_inference-{self.region}-{self.user_key}')
    ...
    def _format_jobdef_name(self, runtime_name, runtime_memory, version=__version__):
        ...
        return f'lithops_inference-worker-{self.user_key}-{version.replace(".", "")}-{name_hash}'
    ...
    def _get_default_runtime_image_name(self):
        ...
        return utils.get_default_container_name(
            self.name, self.config, 'lithops_inference-codeenigne-default'
        )
    ...
    def list_runtimes(self, docker_image_name='all'):
        ...
        for jobdef in jobdefs['items']:
            try:
                if not jobdef['metadata']['name'].startswith(f'lithops_inference-worker-{self.user_key}'):
                    continue
                if not jobdef['metadata']['labels']['type'] == 'lithops_inference-runtime':
                    continue
    ...
    def clear(self, job_keys=None):
        ...
        for job_key in jobs_to_delete:
            try:
                jobrun_name = f'lithops_inference-{job_key.lower()}'
                self.custom_api.delete_namespaced_custom_object(
                    group=config.DEFAULT_GROUP,
                    version=config.DEFAULT_VERSION,
                    name=jobrun_name,
    ...
    def invoke(self, docker_image_name, runtime_memory, job_payload):
        ...
        activation_id = f'lithops_inference-{job_key.lower()}'
    ...
    def _create_container_registry_secret(self):
        ...
        secret = client.V1Secret(
            ...
            data=data,
            kind="Secret",
            metadata=dict(name="lithops_inference-regcred", namespace=self.namespace),
            type="kubernetes.io/dockerconfigjson",
        )

        try:
            self.core_api.delete_namespaced_secret("lithops_inference-regcred", self.namespace)
        except ApiException:
            pass
    ...
    def _generate_runtime_meta(self, docker_image_name, memory):
        ...
        jobrun_name = 'lithops_inference-runtime-metadata'
        ...
        config_map_name = f'lithops_inference-{jobdef_name}-metadata'
    ...
    def _create_config_map(self, config_map_name, payload):
        ...
        cmap.data = {}
        cmap.data["lithops_inference.payload"] = utils.dict_to_b64str(payload)
        ...
        self.core_api.create_namespaced_config_map(
            namespace=self.namespace,
            body=cmap,
            field_manager='lithops_inference'
        )
