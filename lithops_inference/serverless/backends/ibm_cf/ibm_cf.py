...
from lithops_inference.libs.openwhisk.client import OpenWhiskClient
from lithops_inference.constants import CACHE_DIR, COMPUTE_CLI_MSG
...
class IBMCloudFunctionsBackend:
    ...
    def __init__(self, cf_config, internal_storage):
        ...
        self.namespace_name = cf_config.get('namespace', f'lithops_inference-{self.region}-{self.user_key}')
    ...
    def clean(self, all=False):
        ...
        for pkg in packages:
            if pkg['name'].startswith('lithops_inference') and pkg['name'].endswith(self.user_key):
