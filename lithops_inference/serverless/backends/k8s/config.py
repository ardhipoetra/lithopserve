...
DEFAULT_GROUP = "batch"
DEFAULT_VERSION = "v1"
MASTER_NAME = "lithops_inference-master"
MASTER_PORT = 8080
...
kind: Job
metadata:
  name: lithops_inference-worker-name
  namespace: default
  labels:
    type: lithops_inference-worker
    version: lithops_vX.X.X
    user: lithops_inference-user
spec:
    ...
    spec:
      ...
      containers:
        - name: "lithops_inference"
          ...
          command: ["python3"]
          args:
            - "/lithops_inference/lithopsentry.py"
            - "$(ACTION)"
            - "$(DATA)"
          env:
            - name: ACTION
