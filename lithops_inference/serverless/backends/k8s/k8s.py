...
from lithops_inference.constants import COMPUTE_CLI_MSG, JOBS_PREFIX
...
class KubernetesBackend:
    ...
    def _get_default_runtime_image_name(self):
        ...
        return utils.get_default_container_name(
            self.name, self.k8s_config, 'lithops_inference-kubernetes-default'
        )
    ...
    def clean(self, all=False):
        ...
        Deletes all jobs
        """
        logger.debug('Cleaning lithops_inference resources in kubernetes')
        ...
        try:
            ...
            for job in jobs.items:
                if job.metadata.labels['type'] == 'lithops_inference-worker'\
    ...
    def clear(self, job_keys=None):
        ...
        for job_key in jobs_to_delete:
            job_name = f'lithops_inference-{job_key.lower()}'
            logger.debug(f'Deleting job {job_name}')
            try:
                self.batch_api.delete_namespaced_job(
                    name=job_name,
    ...
    def _create_pod(self, pod, pod_name, cpu, memory):
        pod["metadata"]["name"] = f"lithops_inference-pod-{pod_name}"
        ...
        pod["metadata"]["labels"] = {"app": "lithops_inference-pod"}
    ...
    def _delete_workers(self):
        list_pods = self.core_api.list_namespaced_pod(self.namespace, label_selector="app=lithops_inference-pod")
    ...
    def _has_config_changed(self, runtime_mem):
        ...
        for pod in list_pods.items:
            ...
            if multiples_pods_per_node:
                # Is lithops_inference pod with granularity and the user doesn't want it
