...
from lithops_inference.config import default_storage_config, load_yaml_config, extract_storage_config
from lithops_inference.constants import JOBS_PREFIX, TEMP_PREFIX, LOGS_PREFIX, RUNTIMES_PREFIX
...
class CloudStorage(Storage):
    def __init__(self, config=None):
        if isinstance(config, str):
            ...
            if 'lithops_inference' in config:
                self._config = extract_storage_config(config)
            else:
                self._config = config
        else:
