...
from lithops_inference.constants import COMPUTE_CLI_MSG, CACHE_DIR, SA_DATA_FILE
from lithops_inference.config import load_yaml_config, dump_yaml_config
from lithops_inference.standalone.utils import StandaloneMode
...
class AzureVMSBackend:
    ...
    def _create_vnet(self):
        ...
        self.vnet_name = self.config.get('vnet_name', f'lithops_inference-vnet-{str(uuid.uuid4())[-6:]}')
    ...
    def _create_security_group(self):
        ...
        security_group_name = 'lithops_inference-security-group'
    ...
    def _create_master_instance(self):
        ...
        """
        name = self.config.get('master_name') or f'lithops_inference-master-{self.vnet_key}'
    ...
    def _delete_vm_instances(self, all=False):
        ...
        vms_prefixes = ('lithops_inference-worker', 'lithops_inference-master') if all else ('lithops_inference-worker',)

        instances_to_delete = []
        vms_info = self.compute_client.virtual_machines.list(self.config['resource_group'])
        for vm in vms_info:
            if 'type' in vm.tags and vm.tags['type'] == 'lithops_inference-runtime' \
...
class VMInstance:
    ...
    def _create_instance(self, user_data=None):
        ...
        vm_parameters = {
            'location': self.location,
            'tags': {
                'type': 'lithops_inference-runtime',
