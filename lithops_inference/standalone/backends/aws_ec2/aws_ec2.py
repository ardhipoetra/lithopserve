...
from lithops_inference.standalone.utils import CLOUD_CONFIG_WORKER, CLOUD_CONFIG_WORKER_PK, StandaloneMode, get_host_setup_script
from lithops_inference.standalone import LithopsValidationError
...
DEFAULT_LITHOPS_IMAGE_NAME = 'lithops_inference-ubuntu-jammy-22.04-amd64-server'
...
class AWSEC2Backend:
    ...
    def _create_vpc(self):
        ...
        self.vpc_name = self.config.get('vpc_name', f'lithops_inference-vpc-{self.user_key}-{str(uuid.uuid4())[-6:]}')
    ...
    def init(self):
        ...
        if self.mode == StandaloneMode.CONSUME.value:
            ...
            if not self.ec2_data or ins_id != self.ec2_data.get('instance_id'):
                ...
                self.config['master_name'] = 'lithops_inference-consume'
