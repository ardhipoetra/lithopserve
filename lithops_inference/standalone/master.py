...
from lithops_inference.standalone.keeper import BudgetKeeper
from lithops_inference.config import extract_standalone_config
from lithops_inference.standalone.standalone import StandaloneHandler
from lithops_inference.version import __version__ as lithops_version
...
logger = logging.getLogger('lithops_inference.standalone.master')
...
def setup_worker(standalone_handler, worker_info, work_queue_name):
    ...
    try:
        logger.debug(f'Uploading lithops_inference files to {worker}')
        worker.get_ssh_client().upload_local_file(
            '/opt/lithops_inference/lithops_standalone.zip',
        ...
        worker.status = WorkerStatus.ERROR.value
        worker.err = f'Unable to setup lithops_inference in the VM: {str(e)}'
        raise e
