...
from lithops_inference.version import __version__ as lithops_ver
from lithops_inference.utils import sizeof_fmt, is_unix_system, b64str_to_bytes
from lithops_inference.constants import MODULES_DIR
...
def get_function_and_modules(job, internal_storage):
    ...
    backend = job.config['lithops_inference']['backend']
    if job.config[backend].get('runtime_include_function'):
        ...
        func_path = '/'.join(['/opt/lithops_inference', job.func_key])
...
def get_runtime_metadata():
    """
    Generates the runtime metadata needed for lithops_inference
    """
    runtime_meta = dict()
