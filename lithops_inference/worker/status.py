...
import lithops_inference.worker
from lithops_inference.utils import sizeof_fmt
from lithops_inference.storage.utils import create_status_key, \
    create_init_key
...
def create_call_status(job, internal_storage):
    ...
    monitoring_backend = job.config['lithops_inference']['monitoring']
    Status = getattr(lithops_inference.worker.status, '{}CallStatus'
...
class RabbitmqCallStatus(StorageCallStatus):
    ...
    def _send(self):
        ...
        for k in range(int(len(executor_keys) / 2)):
            qname = 'lithops_inference-{}'.format('-'.join(executor_keys[0:k * 3 + 2]))
            queues.append(qname)
