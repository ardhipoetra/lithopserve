...
from lithopserve.constants import MONITORING_INTERVAL
...
class RabbitmqMonitor(Monitor):
    ...
    def __init__(
        ...
        self.queue = f'lithopserve-{self.executor_id}'
...
class JobMonitor:
    ...
    def __init__(self, executor_id, internal_storage, config=None):
        ...
        self.internal_storage = internal_storage
        self.config = config
        self.backend = self.config['lithopserve']['monitoring'].lower() if config else 'storage'
    ...
    def start(self, fs, job_id=None, chunksize=None, generate_tokens=False):
        if self.backend == 'storage':
            mi = self.config['lithopserve'].get('monitoring_interval', MONITORING_INTERVAL) \
