...
import lithopserve.worker
from lithopserve.utils import sizeof_fmt
from lithopserve.storage.utils import create_status_key, \
    create_init_key
...
def create_call_status(job, internal_storage):
    ...
    monitoring_backend = job.config['lithopserve']['monitoring']
    Status = getattr(lithopserve.worker.status, '{}CallStatus'
...
class RabbitmqCallStatus(StorageCallStatus):
    ...
    def _send(self):
        ...
        for k in range(int(len(executor_keys) / 2)):
            qname = 'lithopserve-{}'.format('-'.join(executor_keys[0:k * 3 + 2]))
            queues.append(qname)
