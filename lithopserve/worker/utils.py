...
from lithopserve.version import __version__ as lithops_ver
from lithopserve.utils import sizeof_fmt, is_unix_system, b64str_to_bytes
from lithopserve.constants import MODULES_DIR
...
def get_function_and_modules(job, internal_storage):
    ...
    backend = job.config['lithopserve']['backend']
    if job.config[backend].get('runtime_include_function'):
        ...
        func_path = '/'.join(['/opt/lithopserve', job.func_key])
...
def get_runtime_metadata():
    """
    Generates the runtime metadata needed for lithopserve
    """
    runtime_meta = dict()
