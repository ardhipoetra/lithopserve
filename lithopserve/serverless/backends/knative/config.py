...
BUILD_GIT_URL = 'https://github.com/lithopserve-cloud/lithopserve'
...
kind: ServiceAccount
metadata:
  name: lithopserve-build-pipeline
secrets:
- name: dockerhub-user-token
"""
...
spec:
  ...
  params:
    - name: revision
      value: master
    - name: url
      value: https://github.com/lithopserve-cloud/lithopserve
"""
...
kind: TaskRun
metadata:
  name: lithopserve-runtime-from-git
spec:
  serviceAccountName: lithopserve-build-pipeline
  taskRef:
    name: git-source-to-image
  inputs:
    resources:
    ...
    params:
      - name: pathToDockerFile
        value: lithopserve/compute/backends/knative/tekton/Dockerfile.python36
      - name: imageUrl
        value: docker.io/jsampe/lithopserve-knative-v36
      - name: imageTag
        value: latest
"""
