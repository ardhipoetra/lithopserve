...
class KubernetesBackend:
    ...
    def delete_runtime(self, docker_image_name, memory, version=__version__):
        ...
        try:
            ...
            if e.status != 404:
                logger.error(f"Failed to delete secret: lithopserve-regcred. Reason: {str(e)}")
    ...
    def clean(self, all=False):
        ...
        Deletes all jobs
        """
        logger.debug('Cleaning lithopserve resources in kubernetes')
        ...
        try:
            ...
            for job in jobs.items:
                if job.metadata.labels['type'] == 'lithopserve-worker'\
    ...
    def clear(self, job_keys=None):
        ...
        for job_key in jobs_to_delete:
            job_name = f'lithopserve-{job_key.lower()}'
            logger.debug(f'Deleting job {job_name}')
            try:
                self.batch_api.delete_namespaced_job(
                    name=job_name,
    ...
    def _delete_workers(self):
        list_pods = self.core_api.list_namespaced_pod(self.namespace, label_selector="app=lithopserve-pod")
    ...
    def _has_config_changed(self, runtime_mem):
        ...
        for pod in list_pods.items:
            ...
            if multiples_pods_per_node:
                # Is lithopserve pod with granularity and the user doesn't want it
