...
class AWSLambdaBackend:
    ...
    def _format_function_name(self, runtime_name, runtime_memory, version=__version__):
        ...
        fn_name = f'lithopserve-worker-{self.user_key}-{version.replace(".", "")}-{name_hash}'
    ...
    def _list_layers(self):
        ...
        lithops_layers = []
        for layer in layers:
            if 'lithopserve' in layer['LayerName'] and self.user_key in layer['LayerName']:
        ...
        try:
            ...
            if not images:
                raise Exception(f'Runtime {runtime_name} is not present in ECR.'
                                'Consider running "lithopserve runtime build -b aws_lambda ..."')
    ...
    def clean(self, **kwargs):
        ...
        prefix = f'{self.namespace}-lithopserve-worker-{self.user_key}' \
            if self.namespace else f'lithopserve-worker-{self.user_key}'
