...
from lithopserve.tests.tests_main import (
    print_test_functions,
    print_test_groups,
    run_tests
)
from lithopserve.utils import (
    get_mode,
    setup_lithops_logger,
    verify_runtime_name,
    sizeof_fmt
)
from lithopserve.config import (
    default_config,
...
from lithopserve.constants import (
    CACHE_DIR,
    LITHOPS_TEMP_DIR,
    RUNTIMES_PREFIX,
    JOBS_PREFIX,
...
from lithopserve.serverless import ServerlessHandler
from lithopserve.storage.utils import clean_bucket
from lithopserve.standalone import StandaloneHandler
from lithopserve.localhost import LocalhostHandler
...
def set_config_ow(backend, storage=None, runtime_name=None, region=None):
    config_ow = {'lithopserve': {}, 'backend': {}}

    if storage:
        config_ow['lithopserve']['storage'] = storage

    if backend:
        config_ow['lithopserve']['backend'] = backend
        config_ow['lithopserve']['mode'] = get_mode(backend)
...
def clean(config, backend, storage, debug, region, all):
    ...
    # Clean local lithopserve runtime cache
...
def attach(config, backend, start, debug, region):
    ...
    if config['lithopserve']['mode'] != STANDALONE:
        raise Exception('lithopserve attach method is only available for standalone backends. '
                        f'Please use "lithopserve attach -b {set(STANDALONE_BACKENDS)}"')
    ...
    print('Creating SSH Connection to lithopserve master VM')
...
def build(ctx, name, file, config, backend, debug):
    ...
    if config['lithopserve']['mode'] != SERVERLESS:
        raise Exception('"lithopserve runtime build" command is only available for serverless backends')
...
def deploy(name, storage, backend, memory, timeout, config, debug):
    ...
    if config['lithopserve']['mode'] != SERVERLESS:
        raise Exception('"lithopserve runtime deploy" command is only available for serverless backends')
...
def list_runtimes(config, backend, storage, debug):
    ...
    if config['lithopserve']['mode'] != SERVERLESS:
        raise Exception('"lithopserve runtime list" command is only available for serverless backends')
...
def update(name, config, backend, storage, debug):
    ...
    if config['lithopserve']['mode'] != SERVERLESS:
        raise Exception('"lithopserve runtime update" command is only available for serverless backends')
...
@click.option('--version', '-v', default=None, help='lithopserve version')
@click.option('--backend', '-b', default=None, help='compute backend')
@click.option('--storage', '-s', default=None, help='storage backend')
@click.option('--debug', '-d', is_flag=True, help='debug mode')
def delete(name, config, memory, version, backend, storage, debug):
    ...
    if config['lithopserve']['mode'] != SERVERLESS:
        raise Exception('"lithopserve runtime delete" command is only available for serverless backends')
...
def list_jobs(config, backend, region, debug):
    ...
    if config['lithopserve']['mode'] != STANDALONE:
        raise Exception('"lithopserve job list" command is only available for standalone backends. '
                        f'Please use "lithopserve job list -b {set(STANDALONE_BACKENDS)}"')
...
def list_workers(config, backend, region, debug):
    ...
    if config['lithopserve']['mode'] != STANDALONE:
        raise Exception('"lithopserve worker list" command is only available for standalone backends. '
                        f'Please use "lithopserve worker list -b {set(STANDALONE_BACKENDS)}"')
...
def build_image(ctx, name, file, config, backend, region, debug, overwrite):
    ...
    if config['lithopserve']['mode'] != STANDALONE:
        raise Exception('"lithopserve image build" command is only available for standalone backends. '
                        f'Please use "lithopserve image build -b {set(STANDALONE_BACKENDS)}"')
...
def delete_image(ctx, name, config, backend, region, debug):
    ...
    if config['lithopserve']['mode'] != STANDALONE:
        raise Exception('"lithopserve image delete" command is only available for standalone backends. '
                        f'Please use "lithopserve image delete -b {set(STANDALONE_BACKENDS)}"')
...
def list_images(config, backend, region, debug):
    ...
    if config['lithopserve']['mode'] != STANDALONE:
        raise Exception('"lithopserve image build" command is only available for standalone backends. '
                        f'Please use "lithopserve image list -b {set(STANDALONE_BACKENDS)}"')
