...
from lithopserve.standalone.keeper import BudgetKeeper
from lithopserve.config import extract_standalone_config
from lithopserve.standalone.standalone import StandaloneHandler
from lithopserve.version import __version__ as lithops_version
...
logger = logging.getLogger('lithopserve.standalone.master')
...
def setup_worker(standalone_handler, worker_info, work_queue_name):
    ...
    try:
        logger.debug(f'Uploading lithopserve files to {worker}')
        worker.get_ssh_client().upload_local_file(
            '/opt/lithopserve/lithops_standalone.zip',
        ...
        worker.status = WorkerStatus.ERROR.value
        worker.err = f'Unable to setup lithopserve in the VM: {str(e)}'
        raise e
