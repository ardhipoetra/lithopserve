...
from lithopserve.utils import BackendType, is_lithops_worker, create_handler_zip
from lithopserve.constants import SA_SERVICE_PORT, SA_INSTALL_DIR, TEMP_DIR
from lithopserve.standalone.utils import StandaloneMode, LithopsValidationError, get_master_setup_script
...
class StandaloneHandler:
    ...
    def __init__(self, standalone_config):
        ...
        module_location = f'lithopserve.standalone.backends.{self.backend_name}'
    ...
    def _make_request(self, method, endpoint, data=None):
        ...
        if self.is_lithops_worker:
            url = f"http://lithopserve-master:{SA_SERVICE_PORT}/{endpoint}"
            if method == 'GET':
        ...
        try:
            ...
            if resp['response'] != __version__:
                raise LithopsValidationError(
                    ...
                    f"it doesn't match local lithopserve version {__version__}, consider "
                    "running 'lithopserve clean --all' to delete the master instance")
            return True
    ...
    def _validate_master_service_setup(self):
        ...
        """
        logger.debug(f'Validating lithopserve master service is installed on {self.backend.master}')
        ...
        logger.debug(f"Validating lithopserve master service is running on {self.backend.master}")
        out, err = ssh_client.run_remote_command("service lithopserve-master status")
    ...
    def invoke(self, job_payload):
        ...
        if self.exec_mode != StandaloneMode.CONSUME.value:
            ...
            if job_payload['chunksize'] == 0:
                ...
                job_payload['config']['lithopserve']['chunksize'] = job_payload['worker_processes']
        ...
        def create_workers(workers_to_create):
            ...
            with cf.ThreadPoolExecutor(min(workers_to_create, 48)) as ex:
                for vm_n in range(workers_to_create):
                    ...
                    name = f'lithopserve-worker-{executor_id}-{job_id}-{worker_id}'
        ...
        # delete ssh key
        backend = job_payload['config']['lithopserve']['backend']
    ...
    def _setup_master_service(self):
        """
        Setup lithopserve necessary packages and files in master VM instance
        """
        ...
        logger.debug(f'Uploading lithopserve files to {self.backend.master}')
        ...
        logger.debug(f'Executing lithopserve installation process on {self.backend.master}')
