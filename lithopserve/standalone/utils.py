...
MASTER_SERVICE_NAME = 'lithopserve-master.service'
...
WORKER_SERVICE_NAME = 'lithopserve-worker.service'
...
def get_host_setup_script(docker=True):
    """
    Returns the script necessary for installing a lithopserve VM host
    """
    script = f"""#!/bin/bash
    mkdir -p {SA_TMP_DIR};
    ...
    if [[ ! $(pip3 list|grep "lithopserve") ]]; then
    wait_internet_connection;
    echo "--> Installing Lithops python dependencies"
    pip3 install -U pip flask gevent lithopserve[all];
    fi;
    }}
...
def get_master_setup_script(config, vm_data):
    ...
    echo '127.0.0.1 lithopserve-master' >> /etc/hosts;
    }}
...
def get_worker_setup_script(config, vm_data):
    ...
    echo '{vm_data['master_ip']} lithopserve-master' >> /etc/hosts
    """
