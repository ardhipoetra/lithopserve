...
from lithopserve.util.ssh_client import SSHClient
from lithopserve.constants import COMPUTE_CLI_MSG, CACHE_DIR
from lithopserve.config import load_yaml_config, dump_yaml_config
from lithopserve.standalone.utils import (
    CLOUD_CONFIG_WORKER,
...
DEFAULT_LITHOPS_IMAGE_NAME = 'lithopserve-ubuntu-22-04-3-minimal-amd64-1'
...
class IBMVPCBackend:
    ...
    def _create_vpc(self):
        ...
        self.vpc_name = self.config.get('vpc_name', f'lithopserve-vpc-{iam_id}-{str(uuid.uuid4())[-6:]}')
    ...
    def _create_ssh_key(self):
        ...
        keyname = f'lithopserve-key-{str(uuid.uuid4())[-8:]}'
    ...
    def _create_subnet(self):
        ...
        subnet_name = f'lithopserve-subnet-{self.vpc_key}'
        subnet_data = None
    ...
    def _create_gateway(self):
        ...
        gateway_name = f'lithopserve-gateway-{self.vpc_key}'
        gateway_data = None
    ...
    def _get_or_create_floating_ip(self):
        ...
        for fip in floating_ips_info['floating_ips']:
            if fip['name'].startswith("lithopserve-recyclable") and 'target' not in fip \
               and fip['zone']['name'] == self.config['zone_name']:
                fip_data = fip

        if not fip_data:
            floating_ip_name = f'lithopserve-recyclable-{str(uuid.uuid4())[-4:]}'
    ...
    def _create_master_instance(self):
        ...
        """
        name = self.config.get('master_name') or f'lithopserve-master-{self.vpc_key}'
    ...
    def _delete_vm_instances(self, all=False):
        ...
        vms_prefixes = ('lithopserve-worker', 'lithopserve-master', 'building-image') if all else ('lithopserve-worker',)
    ...
    def _delete_ssh_key(self):
        ...
        if "lithopserve-key-" in key_filename:
