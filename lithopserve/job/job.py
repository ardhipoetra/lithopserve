...
from lithopserve.storage.utils import create_func_key, create_data_key, \
    create_job_key, func_key_suffix
from lithopserve.job.serialize import SerializeIndependent, create_module_data
from lithopserve.constants import MAX_AGG_DATA_SIZE, LOCALHOST, \
...
def _create_job(
    ...
    job = SimpleNamespace()
    job.chunksize = chunksize or config['lithopserve']['chunksize']
    job.worker_processes = config[backend]['worker_processes']
    job.execution_timeout = execution_timeout or config['lithopserve']['execution_timeout']
    ...
    exclude_modules_cfg = config['lithopserve'].get('exclude_modules', [])
    include_modules_cfg = config['lithopserve'].get('include_modules', [])
    ...
    # Check data limit
    if 'data_limit' in config['lithopserve']:
        data_limit = config['lithopserve']['data_limit']
    else:
