...
from lithopserve.utils import CURRENT_PY_VERSION, get_mode, get_default_backend
...
def get_default_config_filename():
    ...
    then ~/.lithopserve/config
    and as last resort the global configuration /etc/lithopserve/config
    """
...
def load_config(config_file=None, log=True):
    ...
    if not config_data:  # Set Lithops to Localhost mode
        ...
        config_data = {'lithopserve': {'mode': c.LOCALHOST, 'backend': c.LOCALHOST, 'storage': c.LOCALHOST}}

    return config_data


def get_log_info(config_file=None, config_data=None):
    """ Return lithopserve logging information set in configuration """
    config_data = copy.deepcopy(config_data) or load_config(config_file, log=False)

    if 'lithopserve' not in config_data or not config_data['lithopserve']:
        config_data['lithopserve'] = {}

    cl = config_data['lithopserve']
...
def default_config(config_file=None, config_data=None, config_overwrite={}, load_storage_config=True):
    ...
    if 'lithopserve' in config_overwrite:
        config_data['lithopserve'].update(config_overwrite['lithopserve'])

    backend = config_data['lithopserve'].get('backend')
    mode = config_data['lithopserve'].get('mode')

    if mode and not backend:
        if mode in config_data and 'backend' in config_data[mode]:
            config_data['lithopserve']['backend'] = config_data[mode]['backend']
        else:
            config_data['lithopserve']['backend'] = get_default_backend(mode)
    elif backend:
        config_data['lithopserve']['mode'] = get_mode(backend)
    elif not backend and not mode:
        mode = config_data['lithopserve']['mode'] = c.MODE_DEFAULT
        config_data['lithopserve']['backend'] = get_default_backend(mode)
    ...
    if mode == c.LOCALHOST:
        ...
        if 'execution_timeout' not in config_data['lithopserve']:
            config_data['lithopserve']['execution_timeout'] = c.EXECUTION_TIMEOUT_LOCALHOST_DEFAULT

        if 'storage' not in config_data['lithopserve']:
            config_data['lithopserve']['storage'] = c.LOCALHOST
        ...
        cb_config = importlib.import_module(f'lithopserve.serverless.backends.{backend}.config')
        cb_config.load_config(config_data)

    elif mode == c.STANDALONE:
        logger.debug(f"Loading Standalone backend module: {backend}")
        sb_config = importlib.import_module(f'lithopserve.standalone.backends.{backend}.config')
        sb_config.load_config(config_data)

    if 'monitoring' not in config_data['lithopserve']:
        config_data['lithopserve']['monitoring'] = c.MONITORING_DEFAULT

    if 'execution_timeout' not in config_data['lithopserve']:
        config_data['lithopserve']['execution_timeout'] = c.EXECUTION_TIMEOUT_DEFAULT

    if 'chunksize' not in config_data['lithopserve']:
        config_data['lithopserve']['chunksize'] = config_data[backend]['worker_processes']

    if load_storage_config:
        config_data = default_storage_config(config_data=config_data)
        if config_data['lithopserve']['storage'] == c.LOCALHOST \
           and backend != c.LOCALHOST:
            raise Exception(f'Localhost storage backend cannot be used with {backend}')

    backend = config_data['lithopserve']['backend']
    if config_data[backend].get('runtime_include_function'):
        config_data['lithopserve']['runtime_include_function'] = config_data[backend]['runtime_include_function']
...
def default_storage_config(config_file=None, config_data=None, backend=None):
    ...
    if 'storage' not in config_data['lithopserve']:
        config_data['lithopserve']['storage'] = c.STORAGE_BACKEND_DEFAULT

    if backend:
        config_data['lithopserve']['storage'] = backend

    sb = config_data['lithopserve']['storage']
    logger.debug(f"Loading Storage backend module: {sb}")
    sb_config = importlib.import_module(f'lithopserve.storage.backends.{sb}.config')
    sb_config.load_config(config_data)
...
def extract_standalone_config(config):
    ...
    sa_config[backend]['user_agent'] = f'lithopserve/{__version__}'
